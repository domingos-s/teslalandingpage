const tabs = [
  { key: 'world-politics', label: 'World Politics', query: 'global diplomacy elections conflict', rss: ['https://feeds.bbci.co.uk/news/world/rss.xml', 'https://www.aljazeera.com/xml/rss/all.xml'] },
  { key: 'us-politics', label: 'U.S. Politics', query: 'US congress white house election policy', rss: ['https://rss.politico.com/politics-news.xml', 'https://feeds.npr.org/1001/rss.xml'] },
  { key: 'business', label: 'Business', query: 'markets economy business finance', rss: ['https://www.cnbc.com/id/10001147/device/rss/rss.html', 'https://feeds.a.dj.com/rss/RSSMarketsMain.xml'] },
  { key: 'technology', label: 'Technology', query: 'technology AI software chips', rss: ['https://www.theverge.com/rss/index.xml', 'https://feeds.arstechnica.com/arstechnica/technology-lab'] }
]
...
async function withTimeout(promise, ms = 8000) { let timer; const timeout = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('timeout')), ms) }); return Promise.race([promise, timeout]).finally(() => clearTimeout(timer)) }
...
async function fetchGuardian(query) {
  const url = `https://content.guardianapis.com/search?q=${encodeURIComponent(query)}&page-size=16&show-fields=thumbnail,trailText&order-by=newest&api-key=test`
...
async function fetchRss(rssUrl) {
  ...
  const json = await fetchJson(url)
  return (json.items || []).slice(0, 10).map((item, idx) => ({ id: `rss-${idx}-${item.guid || item.link}`, title: item.title, description: (item.description || '').replace(/<[^>]+>/g, '').slice(0, 220), source: json.feed?.title || 'RSS', publishedAt: item.pubDate || new Date().toISOString(), url: item.link, imageUrl: item.thumbnail || '' }))
}
...
async function load() {
  ...
  await Promise.all(tabs.map(async (tab) => {
    try {
      ...
      const merged = dedupe([...guardian, ...rssResults]).slice(0, 18)
